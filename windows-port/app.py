"""Copysight -- Video insight extractor for Windows/macOS/Linux.

PyWebView entry point: opens a native window with WebView,
...
if platform.system() == "Windows":
    ...
    for _p in _FFMPEG_PATHS:
        if _p not in _current_path and os.path.exists(_p):
            os.environ["PATH"] = _p + os.pathsep + _current_path
...
class Api:
        ...
        def work():
            try:
                ...
                if cached_transcript:
                    ...
                    # Run in subprocess to isolate GPU/transcription crashes
    ...
    # ── Transcription subprocess (crash isolation) ──

    def _transcribe_in_subprocess(self, mp3, lang, model, ctx):
        """Run transcription in a subprocess to isolate crashes.
        If an error occurs, only the subprocess dies — the main app survives.
        ...
        if proc.returncode != 0:
            ...
            if proc.returncode < 0:
                # Negative return = killed by signal
                self._add_stamp(f"Process error (signal {-proc.returncode}). Try again.")
            elif stderr:
    ...
    def reveal_in_finder(self):
        """Open file explorer with the current entry file selected.
        Works on Windows (Explorer), macOS (Finder), and Linux (various file managers).
        """
